...
describe('getPassiveIncome', () => {
	...
	it('should successfully calculate empty photos', () => {
		...
		const passiveIncome = getPassiveIncome(emptyPhotos, lastHourlyRewardYesterday);
	...
	it('should correctly calculate passive income for 1 photo', () => {
		...
		const lastHourlyRewardYesterday = subtractTime(new Date().toUTCString(), { hours: 1 });
		const passiveIncome = getPassiveIncome(emptyPhotos, lastHourlyRewardYesterday);
		expect(passiveIncome).toBe(50);
	});

	it('should correctly calculate passive income for 2 photos and different levels', () => {
		const hourAndHalfFromNow = subtractTime(new Date().toUTCString(), { hours: 1, minutes: 31, seconds: 5 }); // 76
		const threeHoursFromNow = subtractTime(new Date().toUTCString(), { hours: 3 }); // 300
		const photo1 = { ...mockBasePhoto, created_at: hourAndHalfFromNow };
		const photo2 = { ...mockBasePhoto, created_at: threeHoursFromNow, level_at_time: 2 };
		const emptyPhotos: UserPhotoFragment[] = [photo1, photo2];
		const lastHourlyRewardYesterday = subtractTime(new Date().toUTCString(), { hours: 3, minutes: 30 });
		const passiveIncome = getPassiveIncome(emptyPhotos, lastHourlyRewardYesterday);
	...
	it('should correctly calculate passive income for photos taken before last passive income claim', () => {
		const twoHoursFromNow = subtractTime(new Date().toUTCString(), { hours: 2 }); // 100
	...
	it('should not allow to claim more than 3 hours of passive income', () => {
		const twoHoursFromNow = subtractTime(new Date().toUTCString(), { hours: 5 }); // 500
		const photo1 = { ...mockBasePhoto, created_at: twoHoursFromNow, level_at_time: 2 };
		const emptyPhotos: UserPhotoFragment[] = [photo1];
		const lastHourlyRewardYesterday = subtractTime(new Date().toUTCString(), { hours: 4, minutes: 25 });
		const passiveIncome = getPassiveIncome(emptyPhotos, lastHourlyRewardYesterday);
