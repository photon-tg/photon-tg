...
export function TaskModal(props: TaskModalProps) {
  ...
  return (
    <div className={'flex flex-col'}>
      <img className={'mx-auto mb-[5px] w-[100px]'} src={image.url} />
      <span className={'mb-[10px] text-center text-xxl'}>{name}</span>
      ...
      {reward_by_day && (
        <DailyRewardModal
          ...
          isRewardByDayClaimedToday={isRewardByDayClaimedToday}
          daysCompleted={daysCompleted as number}
          days={reward_by_day}
        />
      )}
    </div>
...
function DailyRewardModal(props: DailyRewardModalProps) {
  const { days, taskId, userTaskId, daysCompleted, isRewardByDayClaimedToday } =
    props;
  ...
  return (
    <div className={'flex flex-col gap-y-[10px]'}>
      ...
      >
        {days.map(({ day, reward }) => (
          <div
            key={day}
            className={cn(
              'flex flex-col gap-y-[2px] overflow-hidden rounded bg-light-blue px-[13px] py-[13px]',
              isRewardByDayClaimedToday &&
                day > daysCompleted &&
                'bg-inactive-grey',
            )}
      ...
      <Button
        disabled={isRewardByDayClaimedToday}
        onClick={claimReward}
        variant={'filled'}
      >
        Claim
